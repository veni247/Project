...
t_max = 1000  # femtosecond  500W
t_0 = -3 * delta_t  # femtosecond
e_max = 300  # meV
T20 = 210  # femtosecond
dt = 2  # femtosecond
khi_o = 3  # 0.001, 0.1, 0.5    # Tham số cường độ xung (0.1 - 2)
...
def FT(t, Pt):
    ...
    write_FT(f"Absorption Spectra Coulomb khi0 = {khi_o} T(t) N = {N}.txt", H, alpha, np.real(P), np.imag(P), np.real(E), np.imag(E))
...
def main():
    ...
    epsilon = [(j + 1) * delta_e / 2 for j in range(N)]
    ...
    write_result(f"Data Coulomb khi0 = {khi_o} T(t).txt", steps, epsilon, Y0_real, Y0_imag, Y1_abs)
    write_Nt_Pt(f"Nt Pt Coulomb khi0 = {khi_o} T(t).txt", steps, Nt, Pt, PT, T2_values)
